...
int is_rdfsbase_disabled() {
  int disabled = 1;
  if (!setjmp(trycatch_buf)) {
    asm volatile (INTELASM("rdfsbase rax") ::: "rax");
    disabled = 0;
    try_abort();
  }
  return disabled;
...
int main(int argc, char* argv[]) {
  ...
  if (!is_rdfsbase_disabled()) {
    printf("\033[91m[!] RDFSBASE is still executable from userspace. You need to disable that using the kernel command line 'nofsgsbase'!\033[0m\n");
    return 1;
  }
